...
# 跨端模块目录名
MODULE_KMP="KuiklyWebview"

# Android 原生模块目录名
MODULE_ANDROID="KuiklyWebviewAndroid"
...
function usage() {
  echo ""
  echo " 发布 KuiklyWebview 产物到 Maven 仓库"
  echo ""
...
function build_gradle_args() {
  ...
  # Group ID
  if [[ -n "${GROUP_ID}" ]]; then
    args="${args} -PGROUP_ID=${GROUP_ID}"
  fi

  # Maven 仓库地址
  if [[ -n "${MAVEN_URL}" ]]; then
    args="${args} -PMAVEN_REPO_URL=${MAVEN_URL}"
  fi

  # Maven 认证
  if [[ -n "${MAVEN_USERNAME}" ]]; then
    args="${args} -PMAVEN_USERNAME=${MAVEN_USERNAME}"
  fi
  if [[ -n "${MAVEN_PASSWORD}" ]]; then
    args="${args} -PMAVEN_PASSWORD=${MAVEN_PASSWORD}"
  fi
...
function publishModule() {
  ...
  echo "> [build] ${GRADLEW} :${project}:${task} ${settings_flag} -PkotlinVersion=${kt_version} -PkuiklyCoreVersion=${core_version} -PHARMONY_ENABLED=${harmony_flag} -PMAVEN_VERSION=${build_version} ${common_args}"
